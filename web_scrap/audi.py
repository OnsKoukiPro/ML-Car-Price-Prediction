...
def get_car_urls(start_page, end_page):
    ...
    for page_num in range(start_page, end_page + 1):
        try:
            base_url = f'https://www.automobile.tn/fr/occasion/s=brand%21%3Aaudi/{page_num}'
...
def save_to_csv(start_page, end_page, filename='audi.csv'):
...
if __name__ == "__main__":
    ...
    save_to_csv(start_page=1, end_page=9)  # Adjust `end_page` for more pages maxx = = 9
    print(f"Time taken: {time.time() - start_time:.2f} seconds")

    print("Fin audi")

    # Charger et exécuter le script 2
    with open("bmw.py") as file:
        exec(file.read())