...
model3 = RandomForestRegressor(oob_score=True)  # Enabling OOB score for Random Forest
model4 = GradientBoostingRegressor()

# Evaluate each model
models(model1)
models(model2)  # Now using the Decision Tree with the specified parameters
models(model3)  # Random Forest with OOB score
models(model4)
...
# Evaluate OOB for Random Forest
print(f"Out-Of-Bag (OOB) Score for Random Forest: {model3.oob_score_}")

# Proximity Matrix (for Random Forest)
from sklearn.ensemble import RandomForestClassifier

model_rf = RandomForestClassifier()
model_rf.fit(x_train, y_train)
proximity_matrix = model_rf.apply(x_test)

# Display Proximity Matrix
sns.heatmap(proximity_matrix, cmap='coolwarm', annot=False)
plt.title("Random Forest Proximity Matrix")
plt.show()
